 ...
 */
export function SettlementCard({ settlements }: { settlements: CurrencySettlement[] | null }) {
  return (
    <Card>
      <CardHeader>
                  ...
                  {settlement.balances.map((balance) => (
                    <li key={balance.userId} className="flex items-center justify-between gap-2 text-sm">
                      <AliasLink
                        ...
                        alias={balance.alias}
                        hasLeftGroup={balance.hasLeftGroup}
                      />
                      <Badge variant={balance.netCents > 0 ? "success" : "destructive"}>
                        {balance.netCents > 0
                          ? `le deben ${toAmount(balance.netCents)}`
                          : `debe ${toAmount(balance.netCents)}`}{" "}
                      ...
                      {settlement.transactions.map((transaction, index) => (
                        ...
                        >
                          ...
                          <AliasLink
                            ...
                            hasLeftGroup={transaction.toHasLeftGroup}
                          />
